...
const getPricingPlans = (locale: string, isYearly: boolean): PricingPlan[] => [
  ...
  },
  {
    ...
    features: [
      ...
      { text: t`Unlimited Custom AI partners` },
...
function FeatureTableRow({ row }: { row: FeatureComparisonRow }) {
  const renderCell = (value: boolean | string, highlight = false) => {
    ...
    return (
      <span
        className={`text-sm ${highlight ? 'text-emphasis' : 'text-subtle'}`}
      >
        {value}
      </span>
    );
  ...
  return (
    <tr className="border-b border-black/5 dark:border-white/10">
      <td className="py-2.5 sm:py-4 px-2 md:px-4 text-sm text-subtle">
        {row.feature}
      </td>
...
export function BillingContent() {
  ...
  if (!snapshot) {
    return (
      <div className="rounded-xl sm:rounded-3xl border border-border/60 bg-card/60 px-4 py-8 sm:px-6 sm:py-12 text-center text-sm text-muted-foreground">
        <Trans>
